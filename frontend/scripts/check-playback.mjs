...
try {
  for (const path of ['/world', '/', '/mapbox']) {
    ...
    // The dock is play / pause and speed only: no clock, timeline, restart or activity jump.
    await expect(page.locator('.dock .clock')).toHaveCount(0)
    await expect(page.getByRole('slider', { name: 'Simulation time', exact: true })).toHaveCount(0)
    await expect(page.getByRole('button', { name: 'Restart simulation', exact: true })).toHaveCount(0)
    await expect(page.getByRole('button', { name: 'Jump to active traffic', exact: true })).toHaveCount(0)
    ...
    // The replay never stops: at the horizon it loops back to its active traffic and keeps playing.
    await page.evaluate(() => window.__cityshift.seek(Number.MAX_SAFE_INTEGER))
    const horizon = await time()
    assert.ok(horizon > 0, 'seeking past the end clamps to the horizon')
    await page.evaluate(() => document.activeElement?.blur())
    await page.keyboard.press('Space')
    await expect(page.getByRole('button', { name: 'Pause simulation', exact: true })).toBeEnabled()
    await expect.poll(time).toBeLessThan(horizon)
    await page.evaluate(() => window.__cityshift.seek(Number.MAX_SAFE_INTEGER)) // while playing: the next frame wraps
    await expect.poll(time).toBeLessThan(horizon)
    await expect(page.getByRole('button', { name: 'Pause simulation', exact: true })).toBeEnabled()
    const looped = await time()
    await expect.poll(time).toBeGreaterThan(looped)
    ...
    for (const [name, key] of [['District', '2'], ['Corridor', '3']]) {
      ...
      await page.evaluate(() => document.body.appendChild(Object.assign(document.createElement('input'), { id: 'shortcut-probe' })).focus())
      ...
      await page.evaluate(() => document.getElementById('shortcut-probe')?.remove())
      await page.keyboard.press('Escape')
...
async function checkLivePlayback(browser, base) {
  ...
  // Parked at the end, Resume loops the real replay back to its active traffic instead of stopping.
  await page.evaluate(() => window.__cityshift.seek(Number.MAX_SAFE_INTEGER))
  await page.getByRole('button', { name: 'Resume simulation', exact: true }).click()
  await page.waitForTimeout(200)
  const resumed = await snapshot()
  assert.ok(resumed.playing && resumed.t >= before.activityStart && resumed.t < before.activityStart + 5, `resume at the end should loop to +${before.activityStart}s, got +${resumed.t}s`)
  await page.waitForTimeout(2000)
  assert.ok(movedCars(resumed, await snapshot()) > 0, 'Cars must move after resuming the real replay')
  assert.deepEqual(errors, [])
  console.log(`Live ${before.run}: ${movedCars(before, after)} visible cars moved; autoplay starts at +${before.activityStart}s; pause, resume and looping passed`)
  await page.close()
}

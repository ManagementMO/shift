      ...
      {visible && createPortal(
        <aside ref={panel} className="sim-settings" role="dialog" aria-labelledby={titleId}>
          ...
          {(appearance || showCityAppearance) && <details className="settings-section settings-visuals"><summary><Icon name="display" /><span>Appearance</span><Icon name="chevron" size={14} /></summary>
            ...
            {showCityAppearance && <><div className="settings-row"><label htmlFor={`${titleId}-projection`}>Overview</label><select id={`${titleId}-projection`} value={display.projection} onChange={(e) => display.set({ projection: e.target.value as 'perspective' | 'isometric' })}><option value="perspective">Perspective</option><option value="isometric">Isometric</option></select></div>
            <div className="settings-row"><label htmlFor={`${titleId}-lighting`}>Lighting</label><select id={`${titleId}-lighting`} value={display.lighting} onChange={(e) => display.set({ lighting: e.target.value as 'afternoon' | 'golden' })}><option value="afternoon">Afternoon</option><option value="golden">Golden hour</option></select></div>

...
export default function GodCityUI({ world, active, onHome }: { world: WorldScene | null; active: boolean; onHome: () => void }) {
  ...
  return <>
    ...
    {panel === 'event-active' && status && <ActiveEventPanel status={status} currentTime={t} onClose={close} onFocus={focusEvent} onStop={() => { if (liveStatus) setNotice('A live event runs for its declared duration; it cannot be stopped early.'); else useGodVisuals.getState().removeEvent(status.id) }} responsesEnabled onResponse={action => { if (AGENT_DEMO_LOCKED && action !== 'close-roads' && action !== 'redirect-traffic') setNotice(AGENT_UNAVAILABLE_MESSAGE); else openTool('closure') }} onViewImpact={() => { focusEvent(); close() }} />}

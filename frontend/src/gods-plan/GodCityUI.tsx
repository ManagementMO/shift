...
export default function GodCityUI({ world, active, onHome, recordingControls }: { world: WorldScene | null; active: boolean; onHome: () => void; recordingControls?: ReactNode }) {
  ...
  const arm = () => {
    if (!world || (populationActive ? !populationDefinition : !session)) { setNotice(populationActive ? 'Create or select a native swarm before placing an event.' : 'Wait for the live city to finish loading.'); return }
  ...
  return <>
    <GodChrome recordingControls={recordingControls} city={pack?.name ?? ''} activeTab="live" openTab={panel === 'events' || panel === 'event-config' ? 'events' : panel === 'log' ? 'log' : (panel === 'agents' || tool === 'residents') ? 'agents' : panel === 'analytics' ? 'analytics' : null} activeTool={panel === 'agents' || tool === 'residents' ? 'people' : panel.startsWith('event') || tool === 'closure' || tool === 'development' ? 'events' : tool === 'temperature' || nativeTemperature ? 'weather' : tool === 'population' ? 'population' : tool === 'area' ? 'map' : 'select'} dateLabel="" timeLabel={simClock(t)} weatherLabel="Clear" temperatureLabel={environment ? `${environment.temperature}°C` : '—'} weatherNote="Simulation temperature, not a weather forecast" statusLabel={populationActive ? nativeStatus : statusLabel} agentCount={populationActive ? populationDefinition?.spec.count ?? 0 : counts?.total ?? 0} playing={populationActive ? playing : playing || !ready} speed={useStore.getState().speed} speeds={PLAYBACK_SPEEDS} ready={populationActive ? !!populationReplay : ready} onSpeed={speed => populationActive ? clock.setSpeed(speed) : live.setSpeed(speed)} is2D={display.projection === 'isometric'} command={command} onTab={onTab} onTool={onTool} onHome={onHome} onCommandChange={setCommand} onCommand={() => commandAction(command)} onSuggestion={commandAction} onTogglePlay={() => populationActive ? clock.toggle() : live.toggle()} onView={() => {}} />
    ...
    {panel === 'events' && <EventMenu onClose={close} onSelect={selectEvent} selectedEvent={liveStatus ? liveStatus.kind : chosen ? 'tornado' : 'normal'} supportedEvents={populationActive ? ['tornado','rain','storm','flood','wildfire'] : ['normal','closure','development','tornado','rain','storm','flood','wildfire']} />}

...
describe('storm system lifecycle', () => {
  it('changes damage and height when a placed tornado changes power', () => {
    const engine = new NullEngine()
    const scene = new Scene(engine)
    const world = fixture()
    ...
    const base = { track_id: 'placed', waypoints: [frame.worldToLonLat(4100, 7200)], radius_m: 110, start_s: 100, end_s: 400, modes: [], label: 'visual' }
    const gentle = { ...base, power: 1 }
    storms.setHazards([gentle])
    const first = storms.storm(gentle)!
    expect(first.power).toBe(1)
    expect(first.damage.plan.some((d) => d.collapse)).toBe(false)
    const lowHeight = first.H
    const extreme = { ...base, power: 5 }
    storms.setHazards([extreme])
    const second = storms.storm(extreme)!
    expect(second).not.toBe(first)
    expect(second.power).toBe(5)
    expect(second.H).toBeGreaterThan(lowHeight)
    expect(second.damage.plan.some((d) => d.collapse)).toBe(true)
    expect(second.damage.plan.filter((d) => d.collapse).every((d) => d.tCollapse >= 125 && d.tCollapse <= 175)).toBe(true)
    const later = { ...extreme, track_id: 'stronger-later', start_s: 120, end_s: 420 }
    storms.setHazards([gentle, later])
    expect(storms.storm(later)!.damage.plan.some((d) => d.collapse)).toBe(true)
    const owners = [gentle, later].flatMap((h) => storms.storm(h)!.damage.plan.map((d) => d.key))

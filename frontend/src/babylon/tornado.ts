...
import { cubeVertexData, Damage, DISPLAY_SCALE, planDamage, stormCenter, ThinSet, type StormCenter, type StormPath } from './destruction'
...
export function hazardKey(h: HazardTrack): string {
  return `${h.track_id}|${h.radius_m}|${h.start_s}|${h.end_s}|${h.waypoints.map((w) => w.join(',')).join(';')}`
}
...
export class Storm {
  ...
  constructor(scene: Scene, city: CityMeshes, world: WorldData, path: StormPath, key: string, shadows: ShadowGenerator | null, plan = planDamage(world, path)) {
    this.path = path
    this.R = path.radius
    this.H = funnelHeight(this.R)
    ...
    this.debris.reserve(DEBRIS_COUNT)
  ...
  private pose(t: number, c: StormCenter): void {
    ...
    const count = Math.round(DEBRIS_COUNT * ramp)
    ...
    this.smoke.draw(vortexSmoke({
      x: c.x, z: c.z, radius: this.R, height: this.H,
      direction: c.dir, seconds: (t - this.path.start) / DISPLAY_SCALE,
      strength: ramp, seed: this.seed,
    }))
  }
...
export class StormSystem {
  ...
  storm(h: HazardTrack): Storm | undefined {
    return this.storms.get(hazardKey(h))
  }

  setHazards(hazards: HazardTrack[]): void {
    ...
    for (const [key, h] of ordered) {
      const path = hazardPath(this.frame, h)
      const plan = planDamage(this.world, path).filter((d) => !claimed.has(d.key))
      for (const d of plan) claimed.add(d.key)
      this.storms.set(key, new Storm(this.scene, this.city, this.world, path, key, this.shadows, plan))
    }
  }

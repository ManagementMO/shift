...
export interface MassingTier { y0: number; y1: number; ring: number[]; holes: number[][] }
export interface MassingBuilding { id: string; cat: BuildingCategory; h: number; x: number; z: number; tiers: MassingTier[] }
export interface MassingData { version: number; network_fingerprint: string; source: string; source_url: string; license: string; excluded_osm_ids: string[]; buildings: MassingBuilding[] }
...
export function appendMassing(building: MassingBuilding, facade: Batch, roof: Batch, color: RGB): void {
  ...
  for (const tier of building.tiers) {
    ...
    roof.polygon(tier.ring, tier.holes, tier.y1 + 0.3, roofColor)
    if (tier.y1 !== highest || Math.abs(signedArea(tier.ring)) < 100 || highest < 12) continue
    const top = tier.y1 + 0.3
    roof.walls(tier.ring, tier.holes, top, top + 0.5, [0.69, 0.68, 0.63], 1)
    const unit = interiorBox(tier, highest > 50 ? 2.8 : 1.5)
    if (!unit) continue

...
Status: design proposal; this PR changes documentation only. It depends on
[PR #7](https://github.com/ManagementMO/shift/pull/7), inspected at `1cc8d47`.
Keep implementation on a separate event-integration branch. PR #7's population
scope and the current Babylon renderer remain intact.

## Recommended outcome

An operator places a declared incident in a running resident simulation. Nearby
residents receive a local observation, decide what it means for their current
plans, optionally warn someone, and propose a response. The city validates and
executes that response; the inspector shows the complete causal chain:
...
Paths below refer to the inspected PR #7 head unless otherwise noted.
...
- [PR #8](https://github.com/ManagementMO/shift/pull/8), inspected at `4cc0aee`,
  ...
  shell. It is an open dependency to reconcile, not code assumed to be on #7.
...
| --- | --- |
| `CityEvent` | `event_id`, `run_id`, source command ID, pack/network fingerprint, revision, kind, effective simulated time, lifecycle status, authoritative geometry, affected edges/classes, perception policy, explicit physical effects, policy version. |
...
An event can be observable with no physical restriction. Rain does not become a
flood, and a visible tornado does not create building collapse or casualties.
...
active effects, each resident's knowledge, pending deliveries/interrupts,
...
Add event placement to the **AI residents** run controls, reusing the existing
map/footprint affordances. Show the target run and its current committed time.
Preview → Apply queues an event; show queued/applied/rejected status. If the run
is paused, queue the event and use the existing explicit Resume control.
...
1. **Event authority:** shared typed contracts, native-run event queue, geometry,
   class policy, restriction ownership/expiry, durable receipts and replay schema.
...
4. **Visible demo:** placement, timely coherent publications, knowledge/provenance
   inspector and recorded causal-chain playback in the existing city shell.
...
resident, and a reachable alternate route. Inspect each step of the chain and
its measured movement. Defer moving/destructive tornado physics, casualties,
...
Inspected the source paths above and PR #8's current branch, including scheduler,
bridge validation, SUMO trip invariants and native prompt projection. From
`backend/`, ran the existing regression tests with local JSON storage:
...
Result: **8 passed**. This documents existing behavior and an integration design;
it does not claim that the proposed feature has been implemented. No runtime
code, provider configuration, or population budget ledger changed.

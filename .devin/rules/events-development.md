...
- The UI offers exactly four kinds — park, townhouses, apartments, skyscraper — defined in `frontend/src/development.ts` `BUILDING_KINDS` with fixed, declared assumptions (no capacity or name fields). `land_use` stays the backend semantic (`residential` / `office` / `park`; `school` remains valid for other tooling) and `developmentKind()` infers the kind for saved specs. Flow: pick a tile → the ghost follows the cursor (`developmentHover`) → click places and checks access immediately → Confirm. Opening the tool with no scenario compiles the default base crowd in the background (`ensureBaseScenario`) instead of asking the user to create one.
...
- Confirmed developments blend in: `DevelopmentOverlay` draws them through `architectureBatches()` with the city's own `CityMaterials` (parks: `grass` lawn + `buildVegetation` trees), with no halo, pin, arrows or camera shortcut; only the selected one gets a thin outline as the anchor for its info card. Since the merge with upstream `main`, that card is the restriction/building/development bubble in `frontend/src/shell/AgentBubble.tsx` (the base-building card gained the two-step Delete from `shell/DeleteButton.tsx`; the separate `BuildingCard.tsx` is gone), and pointer handling lives in `main`'s `WorldBabylon` framework: base buildings are picked through `BuildingIndex` (`SyncMap.buildingFacts`, `Selection { kind: 'building', id }`), saved developments through their meshes' `metadata.development_id`, and while a draft is aimed the ghost follows `unprojectGround` with a crosshair cursor and a click places it. Deleting is **in place** by the user's choice: `DELETE /api/scenarios/{sid}/developments/{id}` drops the development and exactly its trips; `POST /api/scenarios/{sid}/demolitions` hides a base-city building/landmark via `ScenarioSpec.demolished` (visual only — base buildings generate no trips). Both rewrite the scenario under the same id (`Store.replace_scenario` / `replace_plan`); `GET /api/runs?scenario_id=` then lists only runs whose `run_id_for` still matches (`Service.current_runs`), so the UI auto-runs the edited scenario. `run_id_for` excludes `created_at`, `label`, `change_set` and `demolished` (`RUN_IDENTITY_EXCLUDES`). Per-building face ranges recorded in `buildCity` let a demolition collapse just that building's vertices (`CityMeshes.hideBuildings`; `buildingAt` remains for tests) and demolished ids are skipped when picking; the Mapbox renderer cannot hide basemap buildings.
- Upstream `main` removed the split-screen Compare mode, the command bar and the `custom` tool, and auto-submits a first SUMO run whenever a scenario without runs is selected (`selectScenario`). Population-aware before/after therefore lives in `frontend/src/shell/BranchOutcomes.tsx` (Transport lens): it fetches the parent scenario's latest completed run on demand and matches trips with `comparison.ts`. Do not reintroduce `compareRunId`/`compareMode`; upstream's `store.test.ts` asserts they are absent.

...
- Confirmed developments blend in: `DevelopmentOverlay` draws them through `architectureBatches()` with the city's own `CityMaterials` (parks: `grass` lawn + `buildVegetation` trees), with no halo, pin, arrows or camera shortcut; only the selected one gets a thin outline as the anchor for the click card (`frontend/src/world/BuildingCard.tsx`). Deleting is **in place** by the user's choice: `DELETE /api/scenarios/{sid}/developments/{id}` drops the development and exactly its trips; `POST /api/scenarios/{sid}/demolitions` hides a base-city building/landmark via `ScenarioSpec.demolished` (visual only — base buildings generate no trips). Both rewrite the scenario under the same id (`Store.replace_scenario` / `replace_plan`); `GET /api/runs?scenario_id=` then lists only runs whose `run_id_for` still matches (`Service.current_runs`), so the UI auto-runs the edited scenario. `run_id_for` excludes `created_at`, `label`, `change_set` and `demolished` (`RUN_IDENTITY_EXCLUDES`). Base buildings are pickable through per-building face ranges recorded in `buildCity` (`CityMeshes.buildingAt` / `hideBuildings`); the Mapbox renderer cannot hide basemap buildings.
